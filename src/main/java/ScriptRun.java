...
public class ScriptRun extends Menu {
    ...
    private static final String comYourPage = "ваша группа - ";
    ...
    public void scriptRun() throws Throwable {
        ...
        do {
            System.out.print(COLOR_PURPLE + "Введи токен группы: " + COLOR_RED);
        ...
        do {
            System.out.print(COLOR_PURPLE + "Введи id группы (\"-\" и цифры): ");
            id = idSc.nextLine();
            id = id.replace(" ","");
        ...
        String method = vk.groups().getById(actor).executeAsString();
        if (method.contains("\":" + id.replace("-","") + ",\"")) { }
        else {
            System.out.println(COLOR_RED + "ERROR: Проверь токен и id, попробуй ещё раз");
            System.out.println(COLOR_GREEN + "Или напиши в токен \"" + COLOR_RED + "exit" + COLOR_GREEN + "\" для выхода в главное меню.");
            System.out.println(COLOR_BLACK + "--------------------------------------------------------------------------");
    ...
    public void start() throws Throwable {
        ...
        try {
            System.out.println(COLOR_PURPLE + "Напиши в id группы \"" + COLOR_RED + "100" + COLOR_PURPLE + "\" для выхода в главное меню.");
            System.out.print(COLOR_GREEN + "Поставь в начале \"-\" и id сообщество(для спама в посте сообщества).\nВведи id страницы/группы где будет проводится спам {0 - твоя группа}: ");
        ...
        /*--------------------------------------------------------------------------------*/
        System.out.println(COLOR_BLACK + "--------------------------------------------------------------------------");
        if (comIdStr > 0 ) System.out.println(COLOR_YELLOW + "Обратите внимание на то что бы страница была открытой.");
            ...
            for (int i = 1; i < comPieces + 1; i++) {
                ...
                if (request.contains("error")) {
                    if (request.contains("14") & request.contains("Captcha needed")) {
                        ...
                        i = i - 1;
                        continue;
                    } else if (request.contains("9") & request.contains("Flood control: too many requests")) {
                        System.out.println(COLOR_RED + "ERROR: появился флуд контроль.(если флуд контроля много вам стоит подождать пока он уйдет)");
                        System.out.println("Продолжение...\nAnti-Flood_control KD 15 sek.");
                        for (int c = 15; c > 0; c--) {
                            Thread.sleep(ThreadLocalRandom.current().nextInt(1000, 2000));
                            System.out.println(c + "...");
                        }
